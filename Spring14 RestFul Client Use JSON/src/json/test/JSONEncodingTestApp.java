...
public class JSONEncodingTestApp {
	...
	public static void main(String[] args) {
		...
		JSONObject jsonObj = new JSONObject(); ///JSON simple
		jsonObj.put("name", "홍길동"); /// {"name" : "홍길동"}
		jsonObj.put("age", new Integer(1000)); /// {"age" : 1000}
		jsonObj.put("address", "서울"); ///{"address" : "서울"}
		//==> 입력값 확인
		System.out.println("JSON Object 확인 : " +jsonObj);
		//==> Data 추출
		System.out.println("==>JSON Object Data 추출 ");
		System.out.println(jsonObj.get("address")); /// put & get
		System.out.println("\n\n");
		...
		JSONObject innerJsonObj = new JSONObject(); /// innerJsonObj 생성
		innerJsonObj.put("name", "inner홍길동"); ///{"name" : "inner홍길동"}
		innerJsonObj.put("age", new Integer(1000)); ///{"age" : "1000"}
		innerJsonObj.put("address", "inner서울"); ///{"address" : "inner서울"}
		jsonObj.put("innerJson", innerJsonObj); ///"innerJson" : innerJsonObj 형태로 jsonObj에 put
		//==> 입력값 확인
		System.out.println("JSON Object 확인 : " +jsonObj);
		//==> Data 추출
		System.out.println("==>JSON Object Data 추출 ");
		System.out.println(jsonObj.get("address"));
		JSONObject returnJsonObj = (JSONObject)jsonObj.get("innerJson"); ///object type 으로 반환
		...
		map.put("직급2", "대리");
		map.put("직급3", "과장");
		jsonObj.put("career", map); ///json에 map을 통채로 put
		//==> 입력값 확인
